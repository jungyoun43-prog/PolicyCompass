...
검토 결과 위에는 **AI 검토 과정**이 4단계로 표시됩니다: ① 이 브라우저에서 규칙 대조 → ② 같은 출처 API로 대조 결과 전송 → ③ 클라우드 LLM이 기준 문구와 기록을 재검토 → ④ 판정·근거·출처 반환. **전송 내역 보기**를 열면 실제로 무엇이 나가는지(약품·이번 처방·환자 컨텍스트·대조 자료)와 무엇이 나가지 않는지(환자 이름·등록번호·연락처·주소·자유 메모)를 그대로 확인할 수 있습니다. 모델이 설정되지 않은 환경에서는 2~3단계를 실행하지 않고 그 사실을 화면에 표시합니다.

출처의 각 항목을 누르면 **근거 원문**이 열리고, 판정을 가른 문구가 강조 표시됩니다. 항목에 마우스를 올리면 그 강조가 더 진해집니다. 요약을 믿는 대신 기준 문구 자체를 환자 기록 옆에서 바로 대조하도록 한 장치입니다.
