...
진단과 처방은 각각 팝업에서 입력합니다.

- 진단: **진단 추가**를 누르면 상병 검색 팝업이 열립니다. 진단명·증상·코드로 검색해 상병을 고르면 그 상병의 코드 후보가 펼쳐지고, 코드를 선택하면 진단 코드와 진단명이 채워집니다. 코드 시스템(KCD-8 · ICD-10), 주상병·부상병 구분, 확정·의증을 확인한 뒤 이번 진료에 담습니다. 목록에 없는 코드는 진단 코드 입력란에 직접 적을 수 있습니다.
- 처방: **약 처방하기**를 누르면 약품 검색 팝업이 열립니다. 검색 결과는 상품명과 성분명만 보여 주고, 코드·인정 상병·기본 용법·인정 일수는 **자세히 보기**에 접어 둡니다.

`src/diagnosis-catalog.js`의 상병 목록은 검토용 축약 참조 목록이며 공식 KCD 마스터 파일이 아닙니다. 자동 코딩이나 진단 판단을 대신하지 않으며, 최종 상병과 코드는 의료진이 확인해 선택합니다.
