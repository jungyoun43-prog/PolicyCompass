...
export async function modelResponse(task, { invalidCode, failureCode, failureMessage }) {
  try {
    ...
    // The client only learns the route's wording; the real cause has to reach
    // the server log or a model outage is undiagnosable.
    console.error(`[api] ${failureCode}`, error);
