...
const CODE_SYSTEM_SHORT_LABELS = [
  [/urn:kr:kcd/i, "KCD"],
  [/icd-10/i, "ICD-10"],
  [/loinc/i, "LOINC"],
];
...
/**
 * Clinician-facing coding: the bare code with a short system tag for the
 * systems clinicians actually reference. Internal namespaces (urn:…, demo
 * codes) never reach the screen — they stay in the stored record and exports.
 */
export function displayCoding(event = {}) {
  if (isInternalExampleCoding(event)) return "";
  const code = String(event.code ?? "").trim();
  if (!code || /^(PC-|DEMO-)/i.test(code)) return "";
  const system = String(event.system ?? "");
  const short = CODE_SYSTEM_SHORT_LABELS.find(([pattern]) => pattern.test(system))?.[1] ?? "";
  return short ? `${short} ${code}` : code;
}
