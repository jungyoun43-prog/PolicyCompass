...
const PROFILE_CLAIM_ASSESSMENT_ID_PATTERN = /^[A-Za-z0-9][A-Za-z0-9_-]{0,79}$/;
...
function normalizeProfileClaimContext(input, { sourceId, asOf }) {
  ...
  if (!PROFILE_CLAIM_ASSESSMENT_ID_PATTERN.test(assessmentId)) throw new TypeError("질환 평가 식별자가 유효하지 않습니다.");
  ...
  const first = createPatient({
    ...
    events: [
      demoEvent("kim-visit-today", "encounter", "AMB", "내분비내과 외래", asOf, {
        ...
        soap: {
          subjective: "\"약은 하루도 안 빠뜨리고 먹었어요. 어지럽거나 두통은 없는데, 저녁만 되면 발이 좀 붓는 것 같아요.\" 최근 3개월 체중 변화 없음. 흡연·음주 없음.",
          objective: "BP 148/94 mmHg (좌측 상완, 좌위 5분 안정 후) · HbA1c 7.1% · LDL 156 mg/dL. 양측 발등 함요부종 없음, 심음 정상, 폐음 깨끗함.",
          assessment: "1. 본태성 고혈압(I10) - 목표 미달, 수축기 20/이완기 10 초과.\n2. 제2형 당뇨병(E11) - HbA1c 7.1%로 목표 7% 근접하나 미달.\n3. 이상지질혈증 의증 - LDL 156 mg/dL, 당뇨 동반으로 목표 100 미만 필요.",
          plan: "1. 혈압: 현재 ARB 유지하고 4주 뒤 재측정, 목표 미달 시 병용요법 검토.\n2. 당뇨: 식사·운동 요법 재교육, 3개월 뒤 HbA1c 재검.\n3. 지질: 공복 지질 재검 후 스타틴 시작 여부 결정.\n4. 가정혈압 아침·저녁 측정 기록 지참하도록 교육함.",
        },
    ...
    events: [
      ...
      demoEvent("lee-copd-encounter", "encounter", "AMB", "호흡기내과 외래", dateBefore(asOf, 92), {
        ...
        soap: {
          ...
          plan: "1. 폐활량검사 결과 확인 후 진단 확정 및 GOLD 단계 평가.\n2. 금연 유지 지지, 인플루엔자 예방접종 안내.\n3. 증상 악화(객담 화농성 변화·호흡곤란 급증) 시 조기 내원 교육.",
        },
      }),
