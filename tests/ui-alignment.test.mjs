...
test("EMR 모바일은 서명 전에 급여 점검을 보여 주고 스크롤 목록 경계를 알린다", async () => {
  const [html, css, script] = await Promise.all([
    ...
    componentMarkup("components/emr/use-horizontal-scroll.js").then(async (hook) => hook + await componentMarkup("components/emr/patient-rail.jsx")),
  ]);
  ...
  assert.match(script, /centerSelectedPatientCard\(patientListRef\.current, selectedPatientId\)/);
});
